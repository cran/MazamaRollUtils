...
Time series analysis often uses rolling calculations such as moving averages.
These functions are straightforward to write in any language
...
widely used package for ordered observations and rolling calculations
...
* `min_valid` -- Integer minimum number of non-`NA` values a window must contain
for that window to return a non-`NA` result. Supplying `min_valid` implies
`NA`-tolerant counting regardless of `na.rm`. The default, `NULL`, applies no
minimum.
...
For statistical clarity, `roll_sd()` and `roll_var()` do not provide `na.rm` or
`min_valid` arguments.

The `roll_nowcast()` function only accepts the `x` argument and is defined as having:
...
the input vector, which makes it easy to align the return values with the input
data. `NA` values appear in two ways: as stretches at one or both ends, where a
full window cannot be formed (governed by `width`, `align` and `by`), and at any
interior position whose window holds too few valid values to satisfy `na.rm` or
`min_valid`.
